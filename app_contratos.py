...
# ==========================================
# 1. CSS MODERNIZADO E CONFIGURAÇÕES
# ==========================================
st.markdown("""
    <style>
...
# ==========================================
def encontrar_id_etapa(etapa_bruta):
    """Buscador Inteligente: Lê textos incompletos da planilha e acha a caixa certa."""
    etapa_bruta = str(etapa_bruta).strip()
    if not etapa_bruta or etapa_bruta.lower() == 'nan': return ""
    
    # 1. Tenta achar exatamente o código
    if etapa_bruta in tradutor_etapas: 
        return tradutor_etapas[etapa_bruta]
        
    # 2. Tenta achar pelo começo (Ex: Lendo "15.1 Analisar", acha o "15.1")
    for k in sorted(tradutor_etapas.keys(), key=len, reverse=True):
        if etapa_bruta.startswith(str(k)):
            return tradutor_etapas[k]
            
    # 3. Tenta achar palavras-chave dentro do nome da caixa
    for k, v in textos.items():
        if v.replace('\n', ' ').strip().lower() in etapa_bruta.lower():
            return k
...
def gerar_fluxograma_individual(etapa_destaque=None):
    ...
    for nome_setor, lista_ids in setores.items():
        for id_caixa in lista_ids:
            ...
            elif id_caixa == etapa_destaque: cor_fundo, cor_borda, cor_fonte = '#FFF176', '#F57F17', 'black' # Amarelo vibrante
            ...
            if id_caixa in ['N_INICIO', 'N_FIM']:
                ...
                # Caixa super destacada com borda grossa e o Novo Marcador Vermelho
...
def gerar_fluxograma_geral(df_dados):
    ...
    for index, row in df_dados.iterrows():
        ...
        id_et = encontrar_id_etapa(etapa_bruta)
...
if modo_visao == "🌎 Consulta Pública":
    ...
    tipo_contrato = st.selectbox("Selecione a modalidade do contrato:", ["Acordo de Parceria", "Protocolo de Intenções (Em Breve)", "ACT (Em Breve)", "Contrato global (Em Breve)"])
    ...
    if tipo_contrato == "Acordo de Parceria":
        ...
        if busca:
            ...
            if not projeto.empty:
                ...
                id_etapa = encontrar_id_etapa(etapa_bruta)
    ...
    if not st.session_state['nap_autenticado']:
        ...
        st.markdown("---")
        tipo_contrato_nap = st.selectbox("Selecione o Dashboard Gerencial que deseja visualizar:", ["Acordos de Parceria", "Protocolo de Intenções (Em Breve)", "ACT (Em Breve)", "Contrato global (Em Breve)"], key="dropdown_interno")
        ...
        if tipo_contrato_nap == "Acordos de Parceria":
            ...
            df_resumo['Fase Atual'] = df_resumo['Etapa_Atual'].apply(lambda x: textos.get(encontrar_id_etapa(str(x).strip().replace('.0','')), "Não identificada").replace('\n', ' '))
            
            csv_dados = df_resumo[['Registro', 'Titulo', 'Coordenador', 'Fase Atual']].to_csv(index=False).encode('utf-8')
            st.download_button(label="📥 Baixar Planilha (Excel/CSV)", data=csv_dados, file_name='Projetos_Ativos_NAP.csv', mime='text/csv')
